<script>
  ...
  const navItems = [
    ...
    { href: '/properties',    label: 'Props',      icon: 'building' },
...
<nav class="fixed bottom-0 left-0 right-0 z-40 md:hidden safe-bottom"
  style="background: rgba(11,13,20,0.92); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border-top: 1px solid var(--border);">
  <div class="flex overflow-x-auto" style="scrollbar-width:none; -ms-overflow-style:none;">
    {#each navItems as item}
      {@const active = $page.url.pathname.startsWith(item.href)}
      <a
        href={item.href}
        class="flex-none flex flex-col items-center justify-center py-2.5 px-3 min-h-[56px] min-w-[58px] transition-all relative"
      >
        {#if active}
        ...
        <span class="text-[9px] font-semibold leading-tight whitespace-nowrap mt-0.5 transition-colors"
